...
// src/functions/dynamo queries/handler.ts
__export(exports, {
  main: () => main
});
...
var dynamo_db = new AWS.DynamoDB.DocumentClient({
  region: "localhost",
...
// src/functions/dynamo queries/handler.ts
var getQueryResults = async (event) => {
  try {
    ...
    const login_user = await DynamoDB2.runQuery(login_params);
    const result_same_pass = await DynamoDB2.runQuery(same_pass);
...
var main = middyfy(getQueryResults);
