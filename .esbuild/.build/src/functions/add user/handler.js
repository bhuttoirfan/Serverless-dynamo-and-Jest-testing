...
// src/functions/add user/handler.ts
__export(exports, {
  addUser: () => addUser,
  main: () => main
});
...
// src/functions/add user/handler.ts
var addUser = async (event) => {
  try {
    const body2 = event.body;
    const params2 = {
      TableName: "User-Table-Irfan",
      Item: __spreadValues({}, body2)
    };
    await DynamoDB2.addUser(params2);
  ...
  await DynamoDB2.addUser(params);
  return formatJSONResponse({
    message: "User successfully added"
  });
};
var main = middyfy(addUser);
